...
private async logAuditEvent(
  ...
  async createTestSecurityEvent() {
    try {
      // Create a test security event
      await this.logSecurityEvent(
        'TEST_EVENT',
        'MEDIUM',
        'Test security event generated by admin',
        '127.0.0.1',
        'Mozilla/5.0 (Windows NT 10.0; Win64; x64)',
        { source: 'admin_panel', test: true }
      );

      // Also create a test audit log
      await this.logAuditEvent(
        'system-admin',
        'system',
        'TEST_AUDIT_LOG',
      ...
      return { message: 'Test events created successfully' };
    } catch (error) {
      this.logger.error('Failed to create test events:', error);
      throw error;
    }
  }
